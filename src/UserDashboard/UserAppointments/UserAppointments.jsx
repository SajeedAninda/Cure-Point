...
const UserAppointments = () => {
  ...
  let handlePay = id => {
    Swal.fire({
      title: 'Do you want to Pay for this Appointment?',
      text: "You won't be able to revert this!",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      ...
      if (result.isConfirmed) {
        axiosInstance.patch(`/appointmentStatus/${id}`).then(res => {
          if (res.data.modifiedCount >= 1) {
            refetch()
            Swal.fire({
              title: 'Paid!',
              text: 'You Have Paid for Your Appointment.',
              icon: 'success'
            })
          }
        })
  ...
  let unpaidAppointments = appointmentData?.filter(
